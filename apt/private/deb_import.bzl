...
def _deb_import_impl(rctx):
    ...
    rctx.file("BUILD", _DEB_IMPORT_BUILD_TMPL)
...
def _deps_str(dependencies, arch, depth, target = None):
    return ("\n" + " " * depth).join([
        '"//%s/%s%s",' % (dep.name, arch, target or "")
...
def _gen_filegroup_select(package_name, architectures, files = False):
    target_name = "files" if files else "data"
    ...
    if len(architectures) == 1 and architectures.keys()[0] == "all":
        ...
        if package.dependencies:
            srcs += """ + [
        {deps}
    ]""".format(deps = _deps_str(package.dependencies, "all", 8, target))
    else:
        selects = []
        ...
        "@platforms//cpu:{cpu}": [
            {deps}
                ...
                deps = _deps_str(package.dependencies, arch, 12, target),
            )
...
""".format(name = "%s_files" % package_name if files else package_name, srcs = srcs)

def package_aliases_build(package_name, architectures):
    aliases = [
        _gen_alias_select(name, package_name, architectures)
        for name in ("files", "data", "control")
    ]

    filegroups = [
        _gen_filegroup_select(package_name, architectures),
        _gen_filegroup_select(package_name, architectures, files = True),
    ]
